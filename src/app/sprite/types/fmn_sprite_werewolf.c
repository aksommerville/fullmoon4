  ...
  switch (itemid) {
    case FMN_ITEM_WAND: switch (qualifier) {
        ...
        case FMN_SPELLID_PUMPKIN: {
            struct fmn_sprite *pumpkin=fmn_sprite_pumpkinize(sprite);
            if (pumpkin) {
              pumpkin->bv[1]=1; // pass_thru_interact
              punked=1;
            }
          } break;
      } break;
    case FMN_ITEM_WAND|0x80: switch (qualifier) { // generated by pumpkin when we refunct
        case FMN_SPELLID_PUMPKIN: punked=0; break;
      } break;
