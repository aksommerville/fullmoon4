...
uint8_t fmn_platform_get_input_device_name(char *dst,uint8_t dsta,uint8_t p) {
  return inmgr_device_name_by_index(dst,dsta,bigpc.inmgr,p);
}

void fmn_platform_begin_input_configuration(uint8_t p) {
  int devid=inmgr_device_devid_by_index(bigpc.inmgr,p);
  if (devid<1) return;
  bigpc.incfg_devid=devid;
  bigpc.incfg_p=p;
  inmgr_live_config_begin(bigpc.inmgr,devid);
}
...
uint8_t fmn_platform_get_input_configuration_state(uint8_t *p,uint8_t *btnid) {
  ...
  *btnid=bigpc.incfg_btnid;
  switch (bigpc.incfg_status) {
    case INMGR_LIVE_CONFIG_NONE: return FMN_INCFG_STATE_NONE;
    case INMGR_LIVE_CONFIG_FAULT: return FMN_INCFG_STATE_FAULT;
    case INMGR_LIVE_CONFIG_PRESS: return FMN_INCFG_STATE_READY;
    case INMGR_LIVE_CONFIG_RELEASE: return FMN_INCFG_STATE_READY;
    case INMGR_LIVE_CONFIG_PRESS_AGAIN: return FMN_INCFG_STATE_CONFIRM;
    case INMGR_LIVE_CONFIG_RELEASE_AGAIN: return FMN_INCFG_STATE_CONFIRM;
    case INMGR_LIVE_CONFIG_SUCCESS: return FMN_INCFG_STATE_NONE;
  }
  return FMN_INCFG_STATE_NONE;
}
